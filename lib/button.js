...
class Button {
    ...
    async run(jid, anggazyy, quoted = {}) {
        const message = {
            ...
            header: proto.Message.InteractiveMessage.Header.create({ title: this._title, subtitle: this._subtitle, hasMediaAttachment: (this._data ? true : false), ...(this._data ? await prepareWAMessageMedia(this._data, { upload: anggazyy.waUploadToServer }) : {}) })
        };
        ...
        await anggazyy.relayMessage(msg.key.remoteJid, msg.message, { messageId: msg.key.id });
        return msg;
    }
}
